...
const Review = ({ data, showDeleteButton, actionDelete }) => {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      // Assuming you have a function to update the local state or refetch the data
      actionEdit(editData);
  ...
  const openEditModal = () => {
    setEditData({ ...data }); // Initialize with current data
  ...
  return (
    <>
      ...
      {editModalOpen && (
        <EditModal
          headerContent={`Edit Review ${data.id}`}
          data={editData}
          setData={setEditData}
...
const EditModal = ({ headerContent, data, setData, close, handleEdit, editing }) => {
  return (
    <ModalLayout labelledBy="edit-title" onClose={close}>
      <ModalHeader>
      ...
      <ModalBody>
          ...
          <Stack size={2} paddingTop={4}>
            <Typography>
              ...
              <ReactStarsRating
                ...
                isHalf={true}
                value={data.score}
                onChange={(value) => setData({ ...data, score: value })}
              />
            </Typography>
          </Stack>
        </Box>
        <Stack horizontal size={4}>
          ...
          <Button
            variant="primary"
            onClick={handleEdit}
            loading={editing ? true : undefined}
          >Save</Button>
        </Stack>
      </ModalBody>
    </ModalLayout>
